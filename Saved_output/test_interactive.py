...
def getNextImage():
    ...
    fits_file_pB = fits.open(f'CME_0_pB\stepnum_0{currentImageIdx}.fits')
    image_data_pB = fits_file_pB[0].data 
    ax.imshow(image_data_pB, cmap="viridis", norm=LogNorm())
...
def point_added_cb(position: Tuple[float, float], klass: str):
    ...
    # print(points)
    points = klicker.get_positions().get("event")
    print(points)
    if(len(points) == 3):
...
def point_removed_cb(position: Tuple[float, float], klass: str, idx):
    ...
    if(len(klicker.get_positions().get("event")) == 2):
        currentCircle.remove()
        currentCircle = None
        currentRadius = None
        currentCenter = None
